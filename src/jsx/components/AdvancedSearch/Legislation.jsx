...
const Legislation = ({ value, id, onChange, className, isPopulated }) => {
  ...
  const {
    ...
    setSkipFetch: setIsSectionSelected,
  } = useDebouncedFetch({
    source: () =>
      `${process.env.SEARCH_API_URL}/legislation/acts/${act}/sections?search=`,
    term: section,
    extraParams: "&start=0&end=5",
  });
  const actions = {
    ...
    <div className={className}>
      ...
      <div className="compound-field">
        <ListBox
          ...
          onSelection={({ section: value = "" }) =>
            onValueChange(value, "section")
          }
        />
      </div>
    </div>
